...
$(x_i,y+i)$ and $(x_j,y_j)$ are concordant if $(x_i-x_j)(y_i-y_j)>0$ and disconcordant if $(x_i-x_j)(y_i-y_j)<0$。

\subsubsection{Kendall's tau}
Kendall's tau可以对变量之间这种联合的一致性进行采样量化。使$\{(x_1,y_1),(x_2,y_2),\dots,(x_n,y_n)\}$表示关于连续随机变量$(X,Y)$的$n$个观测的随机采样。
...
\cref{eq-invervse-rosenblatt}可以写为
\begin{eqnarray}
...
\end{equation}
其中$B$是标准双变量正太累计分布(bivariate normal cumulative distribution)，$\varrho$是$p(u)$和$p(v)$之间的Pearson系数(Pearson coefficient)。从公式\ref{eq-bivariate}可得到：
\begin{equation} 
