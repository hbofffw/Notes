...
\subsubsection{Sklar's Theorem}\cite{Bill2000An}
...
\subsubsection{Fr$\acute{e}$chet-Hoeffding Bounds for Joint Distribution Functions}
...
$\overline{F}(x) = P[X>x] = 1- F(x)$, $\overline{H}(x,y) = P[X>x,Y>y]$，$\overline{H}$的边缘分布分别为$\overline{F}=\overline{H}(x,-\infty),\overline{G}=\overline{H}(-\infty,y)$，根据\cref{thr-sklar}，可得
\begin{eqnarray*}
